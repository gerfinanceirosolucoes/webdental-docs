...
| Verificação de rede | `CF-Connecting-IP` na whitelist de IPs da Mevo (SSM) | IP de origem dentro do CIDR (SSM) |
| Verificação de credencial | `Authorization: Bearer {token}` — secret | Header `X-Api-Key` — secret  |
...
1. **Whitelist de rede por CIDR**: o IP de origem precisa estar dentro do CIDR configurado no SSM Parameter Store:
   - Staging: `/mevo-api/staging/internal-allowed-ips`
   - Produção: `/mevo-api/production/internal-allowed-ips`
...
**Mitigação sugerida:** migrar o cache para Valkey/ElastiCache compartilhado.
